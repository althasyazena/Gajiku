                    ...
                    <tbody class="divide-y divide-gray-100">
                        :@forelse ($salaries as $salary)
                            <tr class="hover:bg-gray-50 transition">
                                ...
                                <td class="px-6 py-4 text-right text-gray-700">Rp. {{ $salary->gaji_pokok }}</td>
                                <td class="px-6 py-4 text-right text-green-600 font-medium">+ Rp {{ $salary->tunjangan_makan + $salary->tunjangan_transportasi }}</td>
                                <td class="px-6 py-4 text-right text-red-500 font-medium">- Rp {{ $salary->potongan }}</td>
                                <td class="px-6 py-4 text-right font-bold text-gray-800">Rp {{ $salary->gaji_bersih }}</td>
                                <td class="px-6 py-4">
                                    <div class="flex items-center justify-center gap-2">
                                        <a href="{{ route('gaji.show', $salary->id) }}" class="px-3 py-1.5 text-xs font-medium text-blue-600 bg-blue-50 rounded-lg hover:bg-blue-100 transition">Detail</a>
                                        <form action="{{ route('gaji.delete', $salary->id) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="px-3 py-1.5 text-xs font-medium text-red-600 bg-red-50 rounded-lg hover:bg-red-100 transition">Hapus</button>
                                        </form>
                                    </div>
                                </td>
                            </tr>
